...
if (caCertificates.size === 1 && certCertificates.size === 1 && keyCertificates.size === 1) {
    console.log('\n✅ 所有文件使用完全相同的证书组合');
} else {
    console.log('\n⚠️  存在证书差异，可能需要进一步调查');
}
...
const report = {
    ...
    details: {
        caCertificates: Array.from(caCertificates.entries()).map(([hash, files]) => ({
            hash,
            fileCount: files.length,
            files: files.slice(0, 100) // 只保存前10个文件名
        })),
        clientCertificates: Array.from(certCertificates.entries()).map(([hash, files]) => ({
            hash,
            fileCount: files.length,
...
fs.writeFileSync('certificate-analysis-report.json', JSON.stringify(report, null, 2));
